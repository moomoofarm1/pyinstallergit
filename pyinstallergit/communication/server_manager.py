...
class ServerManager:
    ...
    def setup_virtual_environments(self):
        ...
        try:
            ...
            # Setup diarization environment (without Label Studio - now separate)
            self._setup_diarization_environment()

            # Setup Label Studio environment (new separate environment)
    ...
    def _setup_diarization_environment(self):
        ...
        Note: Label Studio is now installed in its own separate environment.
        """
        ...
        try:
            ...
            if deps_file.exists():
                ...
                logger.info("Installing diarization packages (Label Studio now in separate environment)...")
    ...
    def _setup_labelstudio_environment(self):
        ...
        try:
            ...
            # Use uv to install dependencies in the venv
            deps_file = Path("configs/labelstudio_env.txt")
            if deps_file.exists():
                cmd = [uv_exe, "pip", "install", "-p", str(python_path), "-r", str(deps_file)]
                result = subprocess.run(cmd, capture_output=True, text=True)
                ...
                if result.returncode != 0:
                    logger.warning(f"uv installation failed, falling back to pip: {result.stderr}")
                    # Fallback to pip installation
                    self._install_deps_with_pip(self.labelstudio_venv, labelstudio_deps)
                else:
                    logger.info("Label Studio dependencies installed successfully with uv")
            else:
                logger.info("Installing Label Studio packages...")
                # Install all dependencies at once using uv (more efficient)
                cmd = [uv_exe, "pip", "install", "-p", str(python_path)] + labelstudio_deps
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)  # 10 minute timeout
                ...
                if result.returncode != 0:
                    ...
                    logger.info("All Label Studio dependencies installed successfully with uv")
